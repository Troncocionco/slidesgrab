...
VAR_THRESHOLD = 16               # Threshold on the squared Mahalanobis distance between the pixel and the model to dec>DETECT_SHADOWS = False            # If true, the algorithm will detect shadows and mark them.
...
def detect_unique_screenshots(video_path, output_folder_screenshot_path):
    ...
    # varThreshold = Threshold on the squared Mahalanobis distance between the pixel and the model to decide whether a >    # detectShadows = If true, the algorithm will detect shadows and mark them. It decreases the speed a bit, so if you>

    fgbg = cv2.createBackgroundSubtractorMOG2(history=FGBG_HISTORY, varThreshold=VAR_THRESHOLD,detectShadows=DETECT_SHA>
